...
Openfuse 是 [Langfuse](https://github.com/langfuse/langfuse) 的一个 developer-preview fork，把分析存储从 ClickHouse 换成了 [GreptimeDB](https://github.com/GreptimeTeam/greptimedb)。Langfuse 的产品、公共 API 和 SDK 都保持不变；GreptimeDB 成为 traces、observations、scores 以及 dashboard 背后分析数据的 source of truth。

> **状态：alpha / developer preview。** 存储迁移已经完成，读路径与上游做了 parity 校验，但请当作预览软件对待。可以用它在 dev 或 staging 环境、用可以承受丢失的数据来评估「跑在 GreptimeDB 上的 Langfuse」。它还不是 production-ready。见[已知限制](docs/known-limitations.md)。
...
部署前请先读[已知限制](docs/known-limitations.md)。
...
首个预览版是 `1.0.0-alpha1`。要直接跑 standalone 发布镜像而不是本地 build，在 `.env` 里固定一个 tag（例如 `OPENFUSE_STANDALONE_IMAGE=tma1ai/openfuse-standalone:1.0.0-alpha1`），再用 `docker compose -f docker-compose.standalone.yml up -d --pull always` 启动。standalone、split web/worker 镜像和 tag 策略的完整说明见[部署文档](docs/deployment.md#published-images-and-tags)。
