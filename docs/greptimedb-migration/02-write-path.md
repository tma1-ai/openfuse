...
- **Faithful replication of original Langfuse semantics (intentional, not a gap)**:
  ...
  - **No S3-era rolling fallback**: this fork removed the worker S3 fallback after switching standard ingestion and eval-generated scores to `raw_events`. Drain old S3-era queues before deploying this branch; new jobs are ref-only and must find their payload in `raw_events`.
