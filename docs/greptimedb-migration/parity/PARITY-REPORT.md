...
- Fork (GreptimeDB v1.1.0): `http://localhost:3000`, image `openfuse-web:local` (base v3.184.1)
...
## Result (final run `mqm2nj9t`)

| status | count | meaning |
|---|---|---|
| **PASS** | **191** | byte-shape identical to upstream |
| **FAIL** | **0** | real divergences — none remaining |
| KNOWN_LIMITATION | 94 | documented engine/config divergences (see `ledger.md`) |
...
generations, datasets, `metrics/daily`, and a **legal metrics matrix** of 252 queries (per-view
measures × type-valid aggregations × dimensions × granularities).

## What was found and fixed (this PR)
