...
describe("isIdempotentReapplyError", () => {
  it("tolerates GreptimeDB errno 1060 (column already exists on ADD COLUMN re-run)", () => {
    ...
    const error = Object.assign(
      new Error("(TableColumnExists): Column type already exists in table observations"),
  ...
  it("does not tolerate other mysql errors (syntax, connection, unknown table)", () => {
    expect(isIdempotentReapplyError(Object.assign(new Error("syntax"), { errno: 1064 }))).toBe(
      false,
    );
    expect(
      isIdempotentReapplyError(Object.assign(new Error("no table"), { errno: 1146 })),
    ).toBe(false);
    expect(
      isIdempotentReapplyError(Object.assign(new Error("ECONNREFUSED"), { code: "ECONNREFUSED" })),
    ).toBe(false);
  });
