...
const EnvSchema = z
  .object({
    ...
    LANGFUSE_GREPTIME_RECONCILIATION_FLEET_CONCURRENCY: z.coerce
      ...
      .positive()
      .default(1),
    // Per-table region-statistics sampler. SST fragmentation is the only lever on by-type dashboard
    // latency, so make it observable. region_statistics is cluster-global; a Redis cooldown lock keeps
    // a single replica emitting each interval rather than every replica publishing identical series.
